...
# 根据dtu gt得到。
# 1. dtu->colmap获得colmap sparse点云；2. process_colmap_to_json。
import os
import cv2
...
def parse_args():
    ...
    parser.add_argument('--data', type=str, default='/data/colmap/scan24', help='dtu scan folder')
...
def load_from_dtu(source: str) -> Tuple[list, dict, dict, dict]:
    ...
    for i in range(n_images):
        ...
        pose_dict[i] = c2w
        image_dict[i] = os.path.join(source, 'image', f'{i:06d}.png')
    
    intrinsic_dict = {'width': 1600, 'height': 1200, 'fx': fx, 'fy': fy, 'cx': cx, 'cy': cy}
