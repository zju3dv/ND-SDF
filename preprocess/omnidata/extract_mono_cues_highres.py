...
from align_utils import align_normal_y, align_y, best_fit_transform,Aligner
...
def get_args():
    ...
    args.add_argument('--input_dir', type=str, default='/data/tankandtemples/advanced/Ballroom/images', help="highres")
    args.add_argument('--task', choices=['normal', 'depth'],default='depth')
...
def save_depth_output(img, depth, output_dir, img_name):
    ...
    vis_depth = ((depth - depth.min()) / (depth.max() - depth.min())*255).astype(np.uint8)
    deoth_rgb = cv2.applyColorMap(vis_depth, cv2.COLORMAP_INFERNO)
    depth_img = np.concatenate([img, cv2.cvtColor(deoth_rgb, cv2.COLOR_BGR2RGB)], axis = 1)
